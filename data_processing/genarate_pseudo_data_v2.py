...
def main():
    ...
    for data_id in data_dir:
        ...
        label_files = os.listdir(os.path.join(parent_dir, 'zoomed_label'))
        with open(os.path.join(parent_dir, 'zoomed_file_name.txt'), 'r') as f:
            file_names = f.readlines()
        for file_name in file_names:
            print(file_name)
            file_name = file_name[:-2] if file_name[-2:] == '\r\n' else file_name[:-1]
            color = cv2.imread(os.path.join(parent_dir, 'zoomed_height_map_color', file_name+'.png'))
            depth = cv2.imread(os.path.join(parent_dir, 'zoomed_height_map_depth', file_name+'.png'),
                               cv2.IMREAD_ANYDEPTH).astype(np.float32)
            ...
            neglect_points = np.loadtxt(os.path.join(parent_dir, 'zoomed_label', file_name+'.object_points.txt')) + pad_size
            if len(neglect_points) == 0:
                continue
            if file_name+'.good.txt' in label_files:
                good_pixel_labels = np.loadtxt(os.path.join(parent_dir, 'zoomed_label', file_name+'.good.txt')) + pad_size
                ...
                for angle_idx in angle_indices:
                    ...
                    for i, angle in enumerate(np.arange(quantified_angle-3, quantified_angle+3, 1)):
                        grasp_label = np.zeros((288, 288, 3), dtype=np.uint8)  # bgr for opencv
                        grasp_label[..., 0] = 255
                        rotated_neglect_points = dp.rotate(neglect_points, (144, 144), (angle / 360.0) * np.pi * 2)
                        rotated_neglect_points = np.round(rotated_neglect_points).astype(np.int)
                        ...
                        rotated_grasp_centers = np.round(rotated_grasp_centers).astype(np.int)
                        for j in range(3):
                            camera_height = np.random.uniform(0.20, 0.63)
                            new_depth = int(camera_height * 1000.0) - (background_depth_value - depth)
                            pad_depth[pad_size:pad_size + 200, pad_size:pad_size + 200] = new_depth
                            gaussianized_label = dp.gaussianize_label(copy.deepcopy(grasp_label),
                                                                      rotated_grasp_centers,
                                                                      camera_height,
                                                                      is_good=True)
                            ...
                            cv2.imwrite(os.path.join(args.output, 'label_map',
                                                     data_id + '-' + file_name + '-{}-{:02d}-{:02d}.png'.format(angle_idx, i, j)),
                                        gaussianized_label)
                            ...
                            np.savetxt(os.path.join(args.output, 'camera_height',
                                                     data_id + '-' + file_name + '-{}-{:02d}-{:02d}.txt'.format(angle_idx, i, j)),
                                       np.array([camera_height*1000.0]))
                            f_p.write(data_id + '-' + file_name + '-{}-{:02d}-{:02d}\n'.format(angle_idx, i, j))
            else:
                bad_pixel_labels = np.loadtxt(os.path.join(parent_dir, 'zoomed_label', file_name + '.bad.txt')) + pad_size
                ...
                for angle_idx in angle_indices:
                    ...
                    for i, angle in enumerate(np.arange(quantified_angle - 5, quantified_angle + 5, 1)):
                        ...
                        rotated_grasp_centers = dp.rotate(grasp_centers, (144, 144), (angle / 360.0) * np.pi * 2)
                        rotated_grasp_centers = np.round(rotated_grasp_centers).astype(np.int)
                        grasp_label = dp.gaussianize_label(grasp_label,
                                                           rotated_grasp_centers,
                                                           camera_height,
                                                           is_good=False)
