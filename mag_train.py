from hparams import create_mag_hparams
...
parser.add_argument('--from_mag_checkpoint',
                    default=False,
                    type=bool,
                    help='Load checkpoint from mag checkpoint or classification checkpoint.')
...
def main():
    ...
    with tf.Graph().as_default():
        with tf.device(tf.train.replica_device_setter(args.task_id)):
            ...
            # optimizer = tf.train.MomentumOptimizer(0.001, momentum=h.momentum)
            train_op = slim.learning.create_train_op(loss, optimizer)
            add_summary(colors, labels, end_points, loss, h)
            summary_op = tf.summary.merge_all()
            if not args.from_mag_checkpoint:
